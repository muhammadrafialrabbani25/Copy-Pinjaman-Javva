...
class EndToEndFlowTest {
    ...
    @Test
    @Order(5)
    @DisplayName("Skenario 2c: Cancel tidak bisa jika funded < 20%")
    void scenario2c_cannotCancel_ifFundedLessThan20Percent() {
        BorrowerDTO iman = registerBorrowerUseCase.execute(new RegisterBorrowerCommand(
                "Iman Santoso", "081111111111", KTP_IMAN,
                "Jl. Merdeka No. 1", 10_000_000L, 700
        ));
        ...
        assertThrows(IllegalArgumentException.class, () ->
                ...
                "Harus throw exception jika funded < 20% dari loan amount"
        );

        // Status loan tidak berubah
        assertEquals(LoanStatus.FUNDING, getLoanStatus(loan.getId()),
                "Status loan tidak boleh berubah jika cancel gagal");
    }
